...
class Soak:
    ...
    async def soak(self, ctx, amount: float):
        ...
        if amount_split == 0:
            await self.bot.say("{} **:warning: {} BWS is not enough to split between {} users! :warning:**".format(ctx.message.author.mention, amount, len_receivers))
            return
        receivers = []
        ...
        long_soak_msg = ":moneybag: {} **Soaked {} BWS on {} [Total {} BWS]** :moneybag:".format(ctx.message.author.mention, str(amount_split), ', '.join([x.mention for x in receivers]), str(amount))

        if len(long_soak_msg) > 2000:
            await self.bot.say(":moneybag: {} **Soaked {} BWS on {} users [{}]** :moneybag:".format(ctx.message.author.mention, str(amount_split), len_receivers, str(amount)))
        else:
